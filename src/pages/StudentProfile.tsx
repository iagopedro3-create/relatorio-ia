...
const STATUS_LABEL: Record<StudentDocument['status'], { label: string; bg: string; color: string }> = {
  ...
  submitted: { label: 'Aguardando visto', bg: '#fff7ed', color: '#c2410c' },
  approved: { label: 'Aprovado', bg: '#dcfce7', color: '#166534' },
  returned: { label: 'Devolvido', bg: '#fee2e2', color: '#991b1b' },
};
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 flex flex-col gap-6">
          <div className="card p-6" style={{ borderTop: '4px solid var(--color-primary)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '2rem', fontSize: '0.85rem' }}>
              {[
                ...
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', borderBottom: '1px solid #f1f5f9', paddingBottom: '0.5rem' }}>
          ...
          <div className="card p-5" style={{ backgroundColor: student.pei_consent_at ? '#f0fdf4' : '#fffbeb', border: `1px solid ${student.pei_consent_at ? '#bbf7d0' : '#fde68a'}` }}>
            <h4 style={{ color: student.pei_consent_at ? '#166534' : '#92400e', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <ShieldCheck size={18} /> LGPD · Consentimento para PEI
            </h4>
            <p style={{ fontSize: '0.85rem', margin: 0, color: student.pei_consent_at ? '#14532d' : '#78350f' }}>
          ...
          <div className="card p-5">
            ...
            {enrollQ.data.map(e => {
              ...
              return (
                <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', padding: '0.4rem 0', borderBottom: '1px solid #f1f5f9' }}>
        ...
        <div className="lg:col-span-2">
          <div className="card p-0" style={{ overflow: 'hidden' }}>
            <div className="p-5 border-b border-slate-200" style={{ backgroundColor: '#f8fafc' }}>
                  ...
                  {docsQ.data.map((doc, idx) => {
                    ...
                    return (
                      <div key={doc.id} style={{ position: 'relative', marginBottom: idx === docsQ.data.length - 1 ? 0 : '2rem', paddingLeft: '2.5rem' }}>
                        <div style={{ position: 'absolute', left: '-5px', top: '4px', width: '14px', height: '14px', borderRadius: '50%', backgroundColor: doc.kind === 'pei' ? 'var(--color-secondary)' : 'var(--color-primary)', border: '3px solid white', boxShadow: '0 0 0 1px #cbd5e1' }} />
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem', gap: '1rem' }}>
                          <h4 style={{ margin: 0, fontSize: '1rem', color: '#1e293b' }}>{doc.kind === 'pei' ? 'PEI' : 'Relatório descritivo'}{doc.period ? ` · ${doc.period}` : ''}</h4>
                          <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#64748b', backgroundColor: '#f1f5f9', padding: '0.2rem 0.6rem', borderRadius: '12px', whiteSpace: 'nowrap' }}>
                            {new Date(doc.updated_at).toLocaleDateString('pt-BR')}
                          </span>
                        </div>
                        <div style={{ backgroundColor: '#f8fafc', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                          <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '0.75rem', fontSize: '0.8rem', color: '#475569', flexWrap: 'wrap' }}>
                          ...
                          <p style={{ fontSize: '0.85rem', color: '#334155', lineHeight: 1.6, margin: 0, fontStyle: 'italic', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }} onClick={() => setOpen(null)}>
          <div className="card" style={{ maxWidth: '800px', width: '100%', maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setOpen(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8' }}><X size={20} /></button>
            </div>
